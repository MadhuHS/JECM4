...
public class DbOperations extends SQLiteOpenHelper {
	...
	public void deleteProfinfofromDb(String deletedata)
	{
		SQLiteDatabase sqldb = getWritableDatabase();
		
		int status = sqldb.delete("Products_Info","Pname = "+"'"+deletedata+"'", null);
		
		if(status==1)
		{
			Toast.makeText(Dbcontext,"Deletion succesfull",Toast.LENGTH_SHORT).show();
		}
		
		else
		{
			Toast.makeText(Dbcontext,"Deletion failed",Toast.LENGTH_SHORT).show();
